        ...
        if self.new_frame_listener is not None:
          if self.new_frame_listener is not None:
            data_dict = {
                ...
                # "marker_set_count": marker_set_count,
                # "unlabeled_markers_count": unlabeled_markers_count,
                "rigid_body_count": rigid_body_count,
                # "skeleton_count": skeleton_count,
                "labeled_marker_count": labeled_marker_count,
                # "timecode": timecode,
                # "timecode_sub": timecode_sub,
                "timestamp": round(timestamp, 3),
                "is_recording": is_recording,
                "tracked_models_changed": tracked_models_changed,
                "rigid_bodies": [],  # List to store detailed rigid body data
            }

            for rb in rigid_body_data.rigid_body_list:
                rigid_body_entry = {
                    "id": rb.id_num,  # Rigid body ID
                    "position": [round(coord, 4) for coord in rb.pos],  # Round position values to 4 decimals
                    "rotation": [round(quat, 4) for quat in rb.rot],  # Round rotation values to 4 decimals

                    "tracking_valid": rb.tracking_valid,  # Tracking validity
                    # "error": rb.error,  # Marker error
                    # "markers": []  # Initialize marker list
                }

            # Append to data_dict
                data_dict["rigid_bodies"].append(rigid_body_entry)
